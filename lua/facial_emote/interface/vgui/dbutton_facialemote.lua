-- "lua\\facial_emote\\interface\\vgui\\dbutton_facialemote.lua"
...
function PANEL:Think( w, h )
	...
	if (  self.zoomEnabled ) then		
		if ( self:IsHovered() and self.zoomVal < self.zoomMax ) then
...
function PANEL:Paint( w, h )
	...
	if ( self.borderOnHovered and self:IsHovered() and self.borderSize > 0 ) then
...
derma.DefineControl( "DButton_FacialEmote", "Derived DButton for facial emote mod", PANEL, "DButton" )