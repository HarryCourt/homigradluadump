...
local chaseMusic = Sound("npc_alternate/panic.mp3")
...
local npc_alternate_acquire_distance =
	CreateConVar("npc_alternate_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which alternate will chase a target.")

local npc_alternate_spawn_protect =
	CreateConVar("npc_alternate_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, alternate will not target players or hide within 200 units of \z
	a spawn point.")

local npc_alternate_attack_distance =
	CreateConVar("npc_alternate_attack_distance", 80, FCVAR_NONE,
	"The reach of alternate's attack.")

local npc_alternate_attack_interval =
	CreateConVar("npc_alternate_attack_interval", 0.2, FCVAR_NONE,
	"The delay between alternate's attacks.")

local npc_alternate_attack_force =
	CreateConVar("npc_alternate_attack_force", 800, FCVAR_NONE,
	"The physical force of alternate's attack. Higher values throw things \z
	farther.")

local npc_alternate_smash_props =
	CreateConVar("npc_alternate_smash_props", 1, FCVAR_NONE,
	"If set to 1, alternate will punch through any props placed in their way.")

local npc_alternate_allow_jump =
	CreateConVar("npc_alternate_allow_jump", 1, FCVAR_NONE,
	"If set to 1, alternate will be able to jump.")

local npc_alternate_hiding_scan_interval =
	CreateConVar("npc_alternate_hiding_scan_interval", 3, FCVAR_NONE,
	"alternate will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant alternates are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_alternate_hiding_repath_interval =
	CreateConVar("npc_alternate_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to alternate's hiding spot will be redetermined every X seconds.")

local npc_alternate_chase_repath_interval =
	CreateConVar("npc_alternate_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of alternate's target will be redetermined every \z
	X seconds.")

local npc_alternate_expensive_scan_interval =
	CreateConVar("npc_alternate_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_alternate_force_download =
	CreateConVar("npc_alternate_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download alternate resources \z
...
if npc_alternate_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("alternate_nag")
util.AddNetworkString("alternate_navgen")
...
local VECTOR_alternate_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_alternate_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_alternate: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other alternates, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_alternate"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "alternateMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_alternate" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why alternate isn't working.
	net.Start("alternate_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_alternate: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_alternate: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_alternate: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_alternate: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_alternate: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, alternate in pairs(ents.FindByClass("npc_alternate")) do
		alternate:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "alternateNavGen", navEndGenerate)
	else
		print("npc_alternate: nav_generate failed to initialize")
...
concommand.Add("npc_alternate_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_alternate: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. alternate doesn't use
	...
	elseif isConsole then
		print("npc_alternate: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_alternate_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_alternate_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_alternate_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_alternate_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. alternate means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_alternate_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_alternate_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_alternate_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_alternate_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_alternate_attack_distance:GetInt()
		...
		local repathInterval = npc_alternate_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_alternate_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_alternate_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_alternate_hiding_repath_interval:GetFloat()
...
local MAT_alternate = Material("npc_alternate/alternate")
killicon.Add("npc_alternate", "npc_alternate/killicon", color_white)
language.Add("npc_alternate", "alternate ")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_alternate: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a alternate.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_alternate_music_volume =
	CreateConVar("npc_alternate_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by alternate. (0-1, where 0 is muted)")

-- If another alternate comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, alternates do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_alternate_PANIC_COUNT alternates are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of alternate.
local MUSIC_alternate_PANIC_COUNT = 8

local MUSIC_alternate_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_alternate_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_alternate") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("alternatePanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_alternate_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_alternate" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_alternate_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear alternate taunt us.
...
local function startTimer()
	if not timer.Exists("alternatePanicMusicUpdate") then
		timer.Create("alternatePanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_alternate)

	-- Get the normal vector from alternate to the player's eyes, and then compute
	...
	-- alternate should only look 1/3 of the way up to the player so that they
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("alternate is studying this map.", "alternateHUD",
...
net.Receive("alternate_navgen", function()
...
local function requestNavGenerate()
	RunConsoleCommand("npc_alternate_learn")
end
...
net.Receive("alternate_nag", function()
