...
local chaseMusic = Sound("npc_komeiji_fumo_enemy/alarm.mp3")
...
local npc_komeiji_fumo_enemy_acquire_distance =
	CreateConVar("npc_komeiji_fumo_enemy_acquire_distance", 999999, FCVAR_NONE,
	"The maximum distance at which komeiji_fumo_enemy will chase a target.")

local npc_komeiji_fumo_enemy_spawn_protect =
	CreateConVar("npc_komeiji_fumo_enemy_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, komeiji_fumo_enemy will not target players or hide within 200 units of \z
	a spawn point.")

local npc_komeiji_fumo_enemy_attack_distance =
	CreateConVar("npc_komeiji_fumo_enemy_attack_distance", 80, FCVAR_NONE,
	"The reach of komeiji_fumo_enemy's attack.")

local npc_komeiji_fumo_enemy_attack_interval =
	CreateConVar("npc_komeiji_fumo_enemy_attack_interval", 0.2, FCVAR_NONE,
	"The delay between komeiji_fumo_enemy's attacks.")

local npc_komeiji_fumo_enemy_attack_force =
	CreateConVar("npc_komeiji_fumo_enemy_attack_force", 800, FCVAR_NONE,
	"The physical force of komeiji_fumo_enemy's attack. Higher values throw things \z
	farther.")

local npc_komeiji_fumo_enemy_smash_props =
	CreateConVar("npc_komeiji_fumo_enemy_smash_props", 1, FCVAR_NONE,
	"If set to 1, komeiji_fumo_enemy will punch through any props placed in their way.")

local npc_komeiji_fumo_enemy_allow_jump =
	CreateConVar("npc_komeiji_fumo_enemy_allow_jump", 1, FCVAR_NONE,
	"If set to 1, komeiji_fumo_enemy will be able to jump.")

local npc_komeiji_fumo_enemy_hiding_scan_interval =
	CreateConVar("npc_komeiji_fumo_enemy_hiding_scan_interval", 3, FCVAR_NONE,
	"komeiji_fumo_enemy will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant komeiji_fumo_enemys are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_komeiji_fumo_enemy_hiding_repath_interval =
	CreateConVar("npc_komeiji_fumo_enemy_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to komeiji_fumo_enemy's hiding spot will be redetermined every X seconds.")

local npc_komeiji_fumo_enemy_chase_repath_interval =
	CreateConVar("npc_komeiji_fumo_enemy_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of komeiji_fumo_enemy's target will be redetermined every \z
	X seconds.")

local npc_komeiji_fumo_enemy_expensive_scan_interval =
	CreateConVar("npc_komeiji_fumo_enemy_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_komeiji_fumo_enemy_force_download =
	CreateConVar("npc_komeiji_fumo_enemy_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download komeiji_fumo_enemy resources \z
	(restart required after changing).\n\z
	WARNING: If this option is disabled, clients will be unable to see or \z
	hear komeiji_fumo_enemy!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_TIMEOUT = 5

if npc_komeiji_fumo_enemy_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("komeiji_fumo_enemy_nag")
util.AddNetworkString("komeiji_fumo_enemy_navgen")
...
local VECTOR_komeiji_fumo_enemy_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_komeiji_fumo_enemy_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_komeiji_fumo_enemy: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other komeiji_fumo_enemys, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_komeiji_fumo_enemy"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "komeiji_fumo_enemyMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_komeiji_fumo_enemy" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why komeiji_fumo_enemy isn't working.
	net.Start("komeiji_fumo_enemy_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_komeiji_fumo_enemy: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_komeiji_fumo_enemy: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_komeiji_fumo_enemy: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_komeiji_fumo_enemy: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_komeiji_fumo_enemy: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, komeiji_fumo_enemy in pairs(ents.FindByClass("npc_komeiji_fumo_enemy")) do
		komeiji_fumo_enemy:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "komeiji_fumo_enemyNavGen", navEndGenerate)
	else
		print("npc_komeiji_fumo_enemy: nav_generate failed to initialize")
...
concommand.Add("npc_komeiji_fumo_enemy_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_komeiji_fumo_enemy: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. komeiji_fumo_enemy doesn't use
	...
	elseif isConsole then
		print("npc_komeiji_fumo_enemy: Beginning nav_generate requested by server console")
	else
		return
	end
	...
	net.Start("komeiji_fumo_enemy_navgen")
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_komeiji_fumo_enemy_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_komeiji_fumo_enemy_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_komeiji_fumo_enemy_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_komeiji_fumo_enemy_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. komeiji_fumo_enemy means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_komeiji_fumo_enemy_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_komeiji_fumo_enemy_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_komeiji_fumo_enemy_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_komeiji_fumo_enemy_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_komeiji_fumo_enemy_attack_distance:GetInt()
		...
		local repathInterval = npc_komeiji_fumo_enemy_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_komeiji_fumo_enemy_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_komeiji_fumo_enemy_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_komeiji_fumo_enemy_hiding_repath_interval:GetFloat()
...
local MAT_komeiji_fumo_enemy = Material("npc_komeiji_fumo_enemy/komeiji_fumo_enemy.png")
killicon.Add("npc_komeiji_fumo_enemy", "npc_komeiji_fumo_enemy/killicon", color_white)
language.Add("npc_komeiji_fumo_enemy", "komeiji_fumo_enemy")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_komeiji_fumo_enemy: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a komeiji_fumo_enemy.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_komeiji_fumo_enemy_music_volume =
	CreateConVar("npc_komeiji_fumo_enemy_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by komeiji_fumo_enemy. (0-1, where 0 is muted)")

-- If another komeiji_fumo_enemy comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, komeiji_fumo_enemys do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_komeiji_fumo_enemy_PANIC_COUNT komeiji_fumo_enemys are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of komeiji_fumo_enemy.
local MUSIC_komeiji_fumo_enemy_PANIC_COUNT = 8

local MUSIC_komeiji_fumo_enemy_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_komeiji_fumo_enemy_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_komeiji_fumo_enemy") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("komeiji_fumo_enemyPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_komeiji_fumo_enemy_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_komeiji_fumo_enemy" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_komeiji_fumo_enemy_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear komeiji_fumo_enemy taunt us.
...
local function startTimer()
	if not timer.Exists("komeiji_fumo_enemyPanicMusicUpdate") then
		timer.Create("komeiji_fumo_enemyPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_komeiji_fumo_enemy)

	-- Get the normal vector from komeiji_fumo_enemy to the player's eyes, and then compute
	...
	-- komeiji_fumo_enemy should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("komeiji_fumo_enemyHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("komeiji_fumo_enemyHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("komeiji_fumo_enemy is studying this map.", "komeiji_fumo_enemyHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "komeiji_fumo_enemyHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "komeiji_fumo_enemyHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "komeiji_fumo_enemyHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "komeiji_fumo_enemyHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("komeiji_fumo_enemy_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "komeiji_fumo_enemyNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. komeiji_fumo_enemy doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_komeiji_fumo_enemy_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for komeiji_fumo_enemy to figure \z
		this map out.\n\z
...
net.Receive("komeiji_fumo_enemy_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! komeiji_fumo_enemy doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet komeiji_fumo_enemy-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! komeiji_fumo_enemy doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to komeiji_fumo_enemy.\n\z
			\n\z
			If you ARE the server host, you can run npc_komeiji_fumo_enemy_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not komeiji_fumo_enemy-compatible!",
			"Ok", nil,
...
--
list.Set("NPC", "npc_komeiji_fumo_enemy", {
	Name = "komeiji_fumo_enemy",
	Class = "npc_komeiji_fumo_enemy",
