...
local chaseMusic = Sound("npc_flandrenextbot/panic.mp3")
...
local npc_flandrenextbot_acquire_distance =
	CreateConVar("npc_flandrenextbot_acquire_distance", 99999, FCVAR_NONE,
	"The maximum distance at which flandrenextbot will chase a target.")

local npc_flandrenextbot_spawn_protect =
	CreateConVar("npc_flandrenextbot_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, flandrenextbot will not target players or hide within 200 units of \z
	a spawn point.")

local npc_flandrenextbot_attack_distance =
	CreateConVar("npc_flandrenextbot_attack_distance", 80, FCVAR_NONE,
	"The reach of flandrenextbot's attack.")

local npc_flandrenextbot_attack_interval =
	CreateConVar("npc_flandrenextbot_attack_interval", 0.2, FCVAR_NONE,
	"The delay between flandrenextbot's attacks.")

local npc_flandrenextbot_attack_force =
	CreateConVar("npc_flandrenextbot_attack_force", 800, FCVAR_NONE,
	"The physical force of flandrenextbot's attack. Higher values throw things \z
	farther.")

local npc_flandrenextbot_smash_props =
	CreateConVar("npc_flandrenextbot_smash_props", 1, FCVAR_NONE,
	"If set to 1, flandrenextbot will punch through any props placed in their way.")

local npc_flandrenextbot_allow_jump =
	CreateConVar("npc_flandrenextbot_allow_jump", 1, FCVAR_NONE,
	"If set to 1, flandrenextbot will be able to jump.")

local npc_flandrenextbot_hiding_scan_interval =
	CreateConVar("npc_flandrenextbot_hiding_scan_interval", 3, FCVAR_NONE,
	"flandrenextbot will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant flandrenextbots are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_flandrenextbot_hiding_repath_interval =
	CreateConVar("npc_flandrenextbot_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to flandrenextbot's hiding spot will be redetermined every X seconds.")

local npc_flandrenextbot_chase_repath_interval =
	CreateConVar("npc_flandrenextbot_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of flandrenextbot's target will be redetermined every \z
	X seconds.")

local npc_flandrenextbot_expensive_scan_interval =
	CreateConVar("npc_flandrenextbot_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_flandrenextbot_force_download =
	CreateConVar("npc_flandrenextbot_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download flandrenextbot resources \z
...
if npc_flandrenextbot_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("flandrenextbot_nag")
util.AddNetworkString("flandrenextbot_navgen")
...
local VECTOR_flandrenextbot_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_flandrenextbot_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_flandrenextbot: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other flandrenextbots, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_flandrenextbot"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "flandrenextbotMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_flandrenextbot" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why flandrenextbot isn't working.
	net.Start("flandrenextbot_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_flandrenextbot: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_flandrenextbot: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_flandrenextbot: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_flandrenextbot: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_flandrenextbot: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, flandrenextbot in pairs(ents.FindByClass("npc_flandrenextbot")) do
		flandrenextbot:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "flandrenextbotNavGen", navEndGenerate)
	else
		print("npc_flandrenextbot: nav_generate failed to initialize")
...
concommand.Add("npc_flandrenextbot_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_flandrenextbot: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. flandrenextbot doesn't use
	...
	elseif isConsole then
		print("npc_flandrenextbot: Beginning nav_generate requested by server console")
	else
		return
	end
	...
	net.Start("flandrenextbot_navgen")
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_flandrenextbot_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_flandrenextbot_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_flandrenextbot_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_flandrenextbot_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. flandrenextbot means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_flandrenextbot_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_flandrenextbot_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_flandrenextbot_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_flandrenextbot_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_flandrenextbot_attack_distance:GetInt()
		...
		local repathInterval = npc_flandrenextbot_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_flandrenextbot_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_flandrenextbot_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_flandrenextbot_hiding_repath_interval:GetFloat()
...
local MAT_flandrenextbot = Material("npc_flandrenextbot/flandrenextbot")
killicon.Add("npc_flandrenextbot", "npc_flandrenextbot/killicon", color_white)
language.Add("npc_flandrenextbot", "flandrenextbot ")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_flandrenextbot: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a flandrenextbot.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_flandrenextbot_music_volume =
	CreateConVar("npc_flandrenextbot_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by flandrenextbot. (0-1, where 0 is muted)")

-- If another flandrenextbot comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, flandrenextbots do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_flandrenextbot_PANIC_COUNT flandrenextbots are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of flandrenextbot.
local MUSIC_flandrenextbot_PANIC_COUNT = 8

local MUSIC_flandrenextbot_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_flandrenextbot_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_flandrenextbot") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("flandrenextbotPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_flandrenextbot_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_flandrenextbot" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_flandrenextbot_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear flandrenextbot taunt us.
...
local function startTimer()
	if not timer.Exists("flandrenextbotPanicMusicUpdate") then
		timer.Create("flandrenextbotPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_flandrenextbot)

	-- Get the normal vector from flandrenextbot to the player's eyes, and then compute
	...
	-- flandrenextbot should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("flandrenextbotHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("flandrenextbotHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("flandrenextbot is studying this map.", "flandrenextbotHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "flandrenextbotHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "flandrenextbotHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "flandrenextbotHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "flandrenextbotHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("flandrenextbot_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "flandrenextbotNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. flandrenextbot doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_flandrenextbot_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for flandrenextbot to figure \z
		this map out.\n\z
...
net.Receive("flandrenextbot_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! flandrenextbot doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet flandrenextbot-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! flandrenextbot doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to flandrenextbot.\n\z
			\n\z
			If you ARE the server host, you can run npc_flandrenextbot_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not flandrenextbot-compatible!",
			"Ok", nil,
...
--
list.Set("NPC", "npc_flandrenextbot", {
	Name = "flandrenextbot",
	Class = "npc_flandrenextbot",
	Category = "TOUHOU_NEXTBOTS",
	AdminOnly = false
})
