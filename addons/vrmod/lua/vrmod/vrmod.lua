-- "addons\\vrmod\\lua\\vrmod\\vrmod.lua"
...
if CLIENT then
	if g_VR.active then timer.Simple(1,function() VRUtilClientStart() end) end//тупорылый инцел блядь как ты кодил в реал тайме сука
	...
	function VRUtilClientStart()
		...
		VRMOD_DisplayInfo = VRMOD_GetDisplayInfo(1,10)
		...
		hook.Add("RenderScene","vrutil_hook_renderscene",function()
			...
			render.PushRenderTarget( g_VR.rt )
				...
				hook.Call("VRMod_PreRender",g_VR.view)
				...
				hook.Call("VRMod_PreRenderRight",g_VR.view)
				render.RenderView(g_VR.view)
