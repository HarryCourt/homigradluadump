-- "addons\\vrmod\\lua\\vrmod\\vrmod_pickup.lua"
...
elseif SERVER then
	...
	local function pickup(ply, bLeftHand, handPos, handAng)
		...
		for k = 1,#entities do local v = entities[k]
			...
			if hook.Call("VRMod_Pickup", nil, ply, v) == false then continue  end
			--
	...
	vrmod.Receive("vrmod_pickup",function(len, ply)
		local bLeftHand = net.ReadBool()
		local bDrop = net.ReadBool()
		if not bDrop then
	...
	/*hook.Add("PlayerTick","physhandphys",function(ply)
		if not vrmod.IsPlayerInVR(ply) or not ply:Alive() or ply.fake then
			if IsValid(ply.vrmodrhand) then ply.vrmodrhand:Remove() end
			if IsValid(ply.vrmodlhand) then ply.vrmodlhand:Remove() end
	
			return
		end
	
		local rhand = ply.vrmodrhand
	
		if not IsValid(rhand) then
			rhand = ents.Create("prop_physics")
			ply.vrmodrhand = rhand
			rhand:SetModel("models/hunter/plates/plate.mdl")
			rhand:Spawn()
			rhand:Activate()
			rhand:SetCollisionGroup(COLLISION_GROUP_PASSABLE_DOOR)
			rhand:GetPhysicsObject():SetMass(20)
			rhand:SetPos(ply:GetPos())
			//rhand:SetNoDraw(true)
		end
		
		local lhand = ply.vrmodlhand
		
		if not IsValid(lhand) then
			lhand = ents.Create("prop_physics")
			ply.vrmodlhand = lhand
			
			lhand:SetModel("models/hunter/plates/plate.mdl")
			lhand:Spawn()
			lhand:Activate()
			lhand:SetCollisionGroup(COLLISION_GROUP_PASSABLE_DOOR)
			lhand:GetPhysicsObject():SetMass(20)
			lhand:SetPos(ply:GetPos())
			//lhand:SetNoDraw(true)
		end
	
		local rhandphys = rhand:GetPhysicsObject()
		
		local targetPos, targetAng = LocalToWorld(Vector(3,-0,0), Angle(90,0,0),vrmod.GetRightHandPos(ply),vrmod.GetRightHandAng(ply))

		if rhand:GetPos():Distance(targetPos) > 10 then rhand:SetPos(targetPos,true) end
		...
		local lhandphys = lhand:GetPhysicsObject()
	
		local targetPos, targetAng = LocalToWorld(Vector(3,-0,0),Angle(90,0,0),vrmod.GetLeftHandPos(ply),vrmod.GetLeftHandAng(ply))

		if lhand:GetPos():Distance(targetPos) > 10 then lhand:SetPos(targetPos,true) end

		lhandphys:SetVelocity(targetVel)
	
		local _,tempangvel = WorldToLocal(Vector(),targetAng,Vector(),lhand:GetAngles())
		targetVel = Vector(tempangvel.roll,tempangvel.pitch,tempangvel.yaw) * 30
		lhandphys:AddAngleVelocity(targetVel-lhandphys:GetAngleVelocity())
	end)
	
	hook.Add("VRMod_Pickup", "physhandnopickup", function (ply, ent)
		if ent == ply.vrmodrhand or ent == ply.vrmodlhand then return false end
	end)*/
end