-- "addons\\vrmod\\lua\\vrmod\\vrmod_sranipal.lua"
...
elseif SERVER then
	...
	vrmod.Receive( "vrmod_requestflexsetup", function( len, ply )
		sendFlexSetup(net.ReadEntity(), ply)
	end)
	
	vrmod.Receive( "vrmod_flexsetup", function( len, ply )
		ply.vrmod_flexsetup = {
			scale = net.ReadUInt(4),
			eye = net.ReadBool(),
			lip = net.ReadBool(),
	...
	vrmod.Receive( "vrmod_flexdata", function( len, ply )
		if not ply.vrmod_flexsetup then return end
