-- "addons\\vrmod\\lua\\vrmod\\vrmod_character.lua"
...
VRMOD_characterInfo = VRMOD_characterInfo or {}
VRMOD_activePlayers = VRMOD_activePlayers or {}
...
local function UpdateIK(ply)
	...
	local charinfo = VRMOD_characterInfo[steamid]
	...
	for i = 1,#VRMOD_characterInfo[steamid].boneorder do
		local bone = VRMOD_characterInfo[steamid].boneorder[i]
		local parent = VRMOD_characterInfo[steamid].boneinfo[bone].parent
		local wpos, wang
		if VRMOD_characterInfo[steamid].boneinfo[bone].name == "ValveBiped.Bip01_L_Clavicle" then
			wpos = L_ClaviclePos
		elseif VRMOD_characterInfo[steamid].boneinfo[bone].name == "ValveBiped.Bip01_R_Clavicle" then
			wpos = R_ClaviclePos
		else
			local parentPos, parentAng = VRMOD_characterInfo[steamid].boneinfo[parent].pos, VRMOD_characterInfo[steamid].boneinfo[parent].ang
			wpos, wang = LocalToWorld(VRMOD_characterInfo[steamid].boneinfo[bone].relativePos, VRMOD_characterInfo[steamid].boneinfo[bone].relativeAng + VRMOD_characterInfo[steamid].boneinfo[bone].offsetAng, parentPos, parentAng)
		end
		if VRMOD_characterInfo[steamid].boneinfo[bone].overrideAng ~= nil then
			wang = VRMOD_characterInfo[steamid].boneinfo[bone].overrideAng
		end
		local mat = Matrix()
		mat:Translate(wpos)
		mat:Rotate(wang)
		VRMOD_characterInfo[steamid].boneinfo[bone].targetMatrix = mat
		VRMOD_characterInfo[steamid].boneinfo[bone].pos = wpos
		VRMOD_characterInfo[steamid].boneinfo[bone].ang = wang
	end
end
...
local function CharacterInit(ply)
	...
	if VRMOD_characterInfo[steamid] and VRMOD_characterInfo[steamid].modelName == pmname then
		return
	end
	...
	print("recreate",steamid)
	VRMOD_characterInfo[steamid] = {
	...
	RecursiveBoneTable2(cm, cm:LookupBone("ValveBiped.Bip01_L_Clavicle"), VRMOD_characterInfo[steamid].boneinfo, VRMOD_characterInfo[steamid].boneorder)
	RecursiveBoneTable2(cm, cm:LookupBone("ValveBiped.Bip01_R_Clavicle"), VRMOD_characterInfo[steamid].boneinfo, VRMOD_characterInfo[steamid].boneorder)
	...
	VRMOD_characterInfo[steamid].bones = {
		fingers = {
	...
	for k,v in pairs(boneNames) do
		...
		VRMOD_characterInfo[steamid].bones[k] = bone
	...
	VRMOD_characterInfo[steamid].modelName = pmname
	
	local claviclePos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_leftClavicle)
	local upperPos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_leftUpperarm)
	local lowerPos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_leftForearm)
	local handPos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_leftHand)
	local thighPos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_leftThigh)
	local calfPos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_leftCalf)
	local footPos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_leftFoot)
	local headPos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_head)
	local spinePos = cm:GetBonePosition(VRMOD_characterInfo[steamid].bones.b_spine)
	
	VRMOD_characterInfo[steamid].clavicleLen = claviclePos:Distance(upperPos)
	VRMOD_characterInfo[steamid].upperArmLen = upperPos:Distance(lowerPos)
	VRMOD_characterInfo[steamid].lowerArmLen = lowerPos:Distance(handPos)
	VRMOD_characterInfo[steamid].upperLegLen = thighPos:Distance(calfPos)
	VRMOD_characterInfo[steamid].lowerLegLen = calfPos:Distance(footPos)
	...
	--	VRMOD_characterInfo[steamid].characterEyeHeight = eyes.Pos.z
	--	VRMOD_characterInfo[steamid].characterHeadToHmdDist = eyes.Pos.x * 2
	--else --otherwise set some ballparks
	--	headPos = headPos - cm:GetPos()
	--	VRMOD_characterInfo[steamid].characterEyeHeight = headPos.z
	--	VRMOD_characterInfo[steamid].characterHeadToHmdDist = 7
	--end
	VRMOD_characterInfo[steamid].characterEyeHeight = 66.8
	VRMOD_characterInfo[steamid].characterHeadToHmdDist = 6.3
	
	VRMOD_characterInfo[steamid].spineLen = (cm:GetPos().z + VRMOD_characterInfo[steamid].characterEyeHeight) - spinePos.z
...
local function BoneCallbackFunc( ply, numbones )
	local steamid = ply:SteamID()
	if not VRMOD_activePlayers[steamid] or not g_VR.net[steamid].lerpedFrame or (ply:InVehicle() and ply:GetVehicle():GetClass() ~= "prop_vehicle_prisoner_pod") then return end
	if ply:GetBoneMatrix(VRMOD_characterInfo[steamid].bones.b_rightHand) then
		ply:SetBonePosition(VRMOD_characterInfo[steamid].bones.b_rightHand, g_VR.net[steamid].lerpedFrame.righthandPos, g_VR.net[steamid].lerpedFrame.righthandAng + Angle(0,0,180))
	end
	...
	if not g_VR.net[steamid].characterAltHead then
		...
		local mtx = ply:GetBoneMatrix(VRMOD_characterInfo[steamid].bones.b_head)
		if mtx then
			mtx:SetAngles(targetAng)
			ply:SetBoneMatrix(VRMOD_characterInfo[steamid].bones.b_head,mtx)
		end
	end
end
...
local function PrePlayerDrawFunc( ply )
	local steamid = ply:SteamID()
	if not VRMOD_activePlayers[steamid] or not g_VR.net[steamid].lerpedFrame then return end
	
	--hide head in first person
	if ply == LocalPlayer() then
		local ep = EyePos()
		local hide = (ep == g_VR.eyePosLeft or ep == g_VR.eyePosRight) and ply:GetViewEntity() == ply
		ply:ManipulateBoneScale(VRMOD_characterInfo[steamid].bones.b_head, hide and zeroVec or Vector(1,1,1))
	end
	
	VRMOD_characterInfo[steamid].preRenderPos = ply:GetPos()
	
	if not ply:InVehicle() then
		VRMOD_characterInfo[steamid].renderPos = g_VR.net[steamid].lerpedFrame.hmdPos + up:Cross(g_VR.net[steamid].lerpedFrame.hmdAng:Right())*-VRMOD_characterInfo[steamid].characterHeadToHmdDist + Angle(0,g_VR.net[steamid].lerpedFrame.characterYaw,0):Forward()*-VRMOD_characterInfo[steamid].horizontalCrouchOffset*0.8
		VRMOD_characterInfo[steamid].renderPos.z = ply:GetPos().z - VRMOD_characterInfo[steamid].verticalCrouchOffset
		ply:SetPos(VRMOD_characterInfo[steamid].renderPos)
	...
	for i = 1,#VRMOD_characterInfo[steamid].boneorder do
		...
		if ply:GetBoneMatrix(bone) then --prevent unwritable bone errors (usually happens when someone opens pac editor while in vr and pac stuff in general)
			ply:SetBoneMatrix(bone, VRMOD_characterInfo[steamid].boneinfo[bone].targetMatrix )
		end
	end
...
local function PostPlayerDrawFunc( ply )
	local steamid = ply:SteamID()
	if not VRMOD_activePlayers[steamid] or not g_VR.net[steamid].lerpedFrame or ply:InVehicle() then return end
	ply:SetPos(VRMOD_characterInfo[steamid].preRenderPos)
end

-------------------------------------------------------------
local function CalcMainActivityFunc( ply, vel )
	if not VRMOD_activePlayers[ply:SteamID()] or ply:InVehicle() then return end
...
function g_VR.StartCharacterSystem(ply)
	...
	VRMOD_characterInfo[steamid].boneCallback = ply:AddCallback("BuildBonePositions",BoneCallbackFunc)
	...
	VRMOD_activePlayers[steamid] = true
end

function g_VR.StopCharacterSystem(steamid)
	if not VRMOD_activePlayers[steamid] then return end

	local ply = player.GetBySteamID(steamid)

	if VRMOD_characterInfo[steamid] then
		if IsValid(ply) then
			for k,v in pairs(VRMOD_characterInfo[steamid].bones) do
				if not isnumber(v) then continue end
				ply:ManipulateBoneAngles( v, Angle(0,0,0))
			end
			
			ply:RemoveCallback("BuildBonePositions",VRMOD_characterInfo[steamid].boneCallback)

			if ply == LocalPlayer() then
				hook.Remove( "VRMod_PreRender", "vrutil_hook_calcplyrenderpos")
				ply:ManipulateBoneScale(VRMOD_characterInfo[steamid].bones.b_head, Vector(1,1,1))
			end
		end
		VRMOD_activePlayers[steamid] = nil
		VRMOD_characterInfo[steamid] = nil
	end

	if table.Count(VRMOD_activePlayers) == 0 then
		if CLIENT then
...
if Initialize then
	for steamid in pairs(VRMOD_activePlayers) do
		g_VR.StopCharacterSystem(steamid)
		g_VR.StartCharacterSystem(player.GetBySteamID(steamid))
	end
end