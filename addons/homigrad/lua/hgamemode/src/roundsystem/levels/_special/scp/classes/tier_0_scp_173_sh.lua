-- "addons\\homigrad\\lua\\hgamemode\\src\\roundsystem\\levels\\_special\\scp\\classes\\tier_0_scp_173_sh.lua"
...
function CLASS.CanLook(ply,pos,shit)
    local tr = {}
...
if CLIENT then
    ...
    hook.Add("HUDPaint","SCP173",function()
        ...
        draw.SimpleText("ALT что-бы моргать","H.18",w / 2,y + 25,white2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
    ...
    function CLASS:HUDPaint()
        ...
        if count > 0 then
            draw.SimpleText("На тебя смотрят : " .. count,"H.18",ScrW() / 2,25,white2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
        end
    end
end
...
function CLASS:Think()
    ...
    if isClient then
        ...
        if SERVER then
            if self.canMove then
                ...
                self:SetAngles(self:GetNWAngle("Angle"))
                self:SetPos(self:GetNWVector("Pos"))
            ...
            if self.canMove then
                if self:KeyDown(IN_ATTACK) then
                        ...
                        for i,ent in pairs(ents.FindInSphere(self:GetPos(),killDis)) do
                            if not ent:GetNoDraw() and JMod.IsDoor(ent) then
                                ...
                                if ent.hpDoor <= 0 then
                                    ...
                                    sound.Emit(ent:EntIndex(),"physics/metal/metal_grate_impact_hard" .. math.random(1,3) .. ".wav",120,200)
                                end
                            end
                        end
                    end
                    ...
                    for i,ply in pairs(player.GetAll()) do
                        ...
                        sound.Emit(ply:EntIndex(),"sfx/scp/173/necksnap1.mp3")
                        sound.Emit(ply:EntIndex(),"sfx/scp/173/necksnap3.mp3")
    ...
    if CLIENT then
        self.FootstepSound = sound.CreatePoint(self,"sfx/scp/173/stonedrag.mp3")
...
function CLASS:CalcView(view)
    ...
    view.origin = self:GetNWVector("Pos") + Vector(0,0,72)
...
function CLASS:PlayerDraw()
    local mdl = GetClientSideModel(self:GetModel())
    mdl:SetRenderOrigin(not self.canMove and self:GetNWVector("Pos",self:GetPos()) or self:GetPos())
    mdl:SetRenderAngles(self.angle or self:GetNWAngle("Angle"))
    mdl:DrawModel()
...
function CLASS:Pain() return false end
function CLASS:DamageImpulse() return false end
function CLASS:DamagePlayerInput(dmginfo) dmginfo:ScaleDamage(0.1) end

function CLASS:PlayerUse() return false end