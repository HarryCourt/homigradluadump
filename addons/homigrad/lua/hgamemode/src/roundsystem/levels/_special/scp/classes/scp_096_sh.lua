-- "addons\\homigrad\\lua\\hgamemode\\src\\roundsystem\\levels\\_special\\scp\\classes\\scp_096_sh.lua"
...
local function getSound(self,name,sndName)
    self[name] = sound.CreatePoint(self,sndName)
    self[name].loop = true
...
if CLIENT then
    ...
    function CLASS.GlobalThink(list)
        if (scp096Here or 0) > 0 then
            CLASS.ScaredSound = sound.CreatePoint(LocalPlayer(),"sfx/music/096chase.mp3")
        ...
        if ply:Alive() and not ply.isSCP then
            Lerp = LerpFT(0.1,Lerp,math.max(scp096Volume or 0,scp096Here or 0))
        else
...
function CLASS.Looking(self)
    ...
    for i,ply in pairs(player.GetAll()) do
        ...
        if CanLook(self,ply:EyePos(),0.4) then players[#players + 1] = ply end
    end
...
function CLASS:Move(mv)
    ...
    local value = (state == 0 and 125) or (state == 1 and 0) or (state == 2 and 400)
...
function CLASS:Think()
    ...
    if state == 0 then
        if CLIENT then
            ...
            scp096Here = sound.GetDSP(self:GetPos()) == DSP_LEVEL0 and 1 or 0
        end
    elseif state == 1 then
    ...
    elseif state == 2 then
        if CLIENT then
            ...
            for i,ply in pairs(player.GetAll()) do
                if not self.playersView[ply] or ply:GetPos():Distance(pos) > killDis then continue end

                if SCP173.IgnoreLook(self,ply) then
