-- "addons\\jmod_master\\lua\\weapons\\wep_jack_gmod_eztoolbox.lua"
...
function SWEP:FindNailPos()
	local pos,ang = self.Owner:Eye()

	local Pos, Vec = pos,ang:Forward()
...
function SWEP:GetPackagableObject()
	...
	local Tr = util.QuickTrace(pos,ang:Forward() * 80, {self.Owner})
...
function SWEP:WhomIlookinAt()
	...
	local Tr = util.QuickTrace(pos,ang:Forward() * 80, Filter)
...
if CLIENT then
	...
	function SWEP:SCKDrawWorldModel()
		...
		for k, name in pairs(self.wRenderOrder) do
			...
			if v.fuckoff and LocalPlayer() == self.Owner then
				if IsValid(v.modelEnt) then v.modelEnt:SetNoDraw(true) end
